...
class BSM_Admin_Handler {
    ...
    public function handle_import_contacts() {
        ...
        if ( ! file_exists( $file_path ) ) {
            set_transient('bsm_admin_notice', ['type' => 'error', 'message' => __('Ficheiro de importação não encontrado. Por favor, tente novamente desde o passo 1.', 'bluesendmail')], 30);
        ...
        global $wpdb;
        $this->plugin->log_event('info', 'import', "Iniciando importação do ficheiro: {$file_path}");
    ...
    public function handle_save_contact() {
		...
		$data = array('email' => $email,'first_name' => sanitize_text_field( $_POST['first_name'] ),'last_name' => sanitize_text_field( $_POST['last_name'] ),'company' => sanitize_text_field( $_POST['company'] ),'job_title' => sanitize_text_field( $_POST['job_title'] ),'segment' => sanitize_text_field( $_POST['segment'] ),'status' => sanitize_key( $_POST['status'] ));
		...
		// MELHORIA: Verifica se o e-mail já existe ANTES de tentar inserir/atualizar.
		$existing_contact = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bluesendmail_contacts WHERE email = %s", $email ) );
		if ( $existing_contact && $existing_contact->contact_id != $contact_id ) {
			set_transient( 'bsm_admin_notice', [ 'type' => 'error', 'message' => __( 'Este e-mail já está cadastrado para outro contato.', 'bluesendmail' ) ], 30 );
			$redirect_url = admin_url( 'admin.php?page=bluesendmail-contacts&action=' . ( $contact_id ? 'edit&contact=' . $contact_id : 'new' ) );
	...
	public function handle_save_automation() {
        ...
        $data = array(
            ...
            'action_meta' => absint( $_POST['action_meta'] ?? 0 ),
        );

        if ( empty($data['name']) || empty($data['trigger_meta']) || empty($data['action_meta']) ) {
            set_transient('bsm_admin_notice', ['type' => 'error', 'message' => __('Todos os campos são obrigatórios.', 'bluesendmail')], 30);
