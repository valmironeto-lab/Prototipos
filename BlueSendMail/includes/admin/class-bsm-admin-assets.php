...
class BSM_Admin_Assets {
    ...
    public function enqueue_assets( $hook ) {
        ...
        $is_automations_page = 'bluesendmail-automations' === $page;
        ...
        if ( $is_automations_page ) { $js_deps[] = 'jquery-ui-sortable'; }
    ...
    private function get_contacts_growth_data() {
		global $wpdb;
		$results = $wpdb->get_results( "SELECT DATE(created_at) AS date, COUNT(contact_id) AS count FROM {$wpdb->prefix}bluesendmail_contacts WHERE created_at >= CURDATE() - INTERVAL 30 DAY GROUP BY DATE(created_at) ORDER BY date ASC" );
		$dates = array();
		for ( $i = 29; $i >= 0; $i-- ) { $dates[ date( 'Y-m-d', strtotime( "-$i days" ) ) ] = 0; }
		foreach ( $results as $result ) { $dates[ $result->date ] = (int) $result->count; }
		return array( 'labels' => array_keys( $dates ), 'data'   => array_values( $dates ) );
	}
    ...
    private function get_performance_data() {
        ...
        return array( 'sent' => (int) $total_sent_emails, 'opens' => (int) $total_unique_opens, 'clicks' => (int) $total_unique_clicks, 'labels' => array( 'not_opened' => __( 'Não Aberto', 'bluesendmail' ), 'opened' => __( 'Abertos (sem clique)', 'bluesendmail' ), 'clicked' => __( 'Clicados', 'bluesendmail' ) ) );
    }
}
