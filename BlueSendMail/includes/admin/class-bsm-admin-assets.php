...
class BSM_Admin_Assets {
    ...
    public function enqueue_assets( $hook ) {
		...
		// MELHORIA: Carregar assets de terceiros localmente.
		if ( $is_campaign_editor || $is_import_page ) {
			wp_enqueue_style( 'select2', BLUESENDMAIL_PLUGIN_URL . 'assets/vendor/select2/select2.min.css', array(), '4.0.13' );
			wp_enqueue_script( 'select2', BLUESENDMAIL_PLUGIN_URL . 'assets/vendor/select2/select2.min.js', array( 'jquery' ), '4.0.13', true );
		}
		if ( $is_reports_page || $is_dashboard_page ) {
			wp_enqueue_script( 'chartjs', BLUESENDMAIL_PLUGIN_URL . 'assets/vendor/chartjs/chart.umd.min.js', array(), '4.4.1', true );
		}
		...
		$script_data = array( 
			...
			'i18n' => array(
				'selectListsPlaceholder' => __( 'Selecione as listas de destinatários', 'bluesendmail' ),
				'templateLoadError'      => __( 'Erro ao carregar o template.', 'bluesendmail' ),
				'communicationError'     => __( 'Erro de comunicação ao carregar o template.', 'bluesendmail' ),
			),
        );
